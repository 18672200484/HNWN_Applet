		...
		public void HandleHomePageData(Action<string, eOutputType> output)
		{
			try
			{
				...
				if (list.Count > 0)
				{
					...
					commonDAO.SetSignalDataValue(GlobalVars.MachineCode_QC_JxSampler_1, "已采车数", list.Count(a => a.SamplePlace == "01").ToString());
					commonDAO.SetSignalDataValue(GlobalVars.MachineCode_QC_JxSampler_2, "已采车数", list.Count(a => a.SamplePlace == "02").ToString());
					commonDAO.SetSignalDataValue(GlobalVars.MachineCode_QC_JxSampler_3, "已采车数", list.Count(a => a.SamplePlace == "03").ToString());
				}
				else
				{
					...
					commonDAO.SetSignalDataValue(GlobalVars.MachineCode_QC_JxSampler_1, "已采车数", "");
					commonDAO.SetSignalDataValue(GlobalVars.MachineCode_QC_JxSampler_2, "已采车数", "");
					commonDAO.SetSignalDataValue(GlobalVars.MachineCode_QC_JxSampler_3, "已采车数", "");
				}
				//IList<CmcsLMYB> yblist = commonDAO.SelfDber.Entities<CmcsLMYB>(" where InFactoryTime>=:dtStart and InFactoryTime<:dtEnd order by InFactoryTime desc", new { dtStart = DateTime.Now.Date, dtEnd = DateTime.Now.Date.AddDays(1) });
				//int ybcount = yblist.Sum(a => a.TransportNumber);
				commonDAO.SetSignalDataValue(GlobalVars.MachineCode_HomePage_1, "在途车数", "无");
				commonDAO.SetSignalDataValue(GlobalVars.MachineCode_HomePage_1, "今日制样数", "无");
				commonDAO.SetSignalDataValue(GlobalVars.MachineCode_HomePage_1, "今日化验数", "无");
				commonDAO.SetSignalDataValue(GlobalVars.MachineCode_HomePage_1, "今日采样桶数", "无");
				commonDAO.SetSignalDataValue(GlobalVars.MachineCode_HomePage_1, "今日存煤量", "无");
				commonDAO.SetSignalDataValue(GlobalVars.MachineCode_HomePage_1, "今日耗煤量", "无");
