...
using CMCS.DumblyConcealer.Tasks.AutoMaker.KYEntities;
...
namespace CMCS.DumblyConcealer.Tasks.AutoMaker
{
    ...
    public class EquAutoMakerDAO
    {
        ...
        /// <param name="output"></param>
        /// <returns></returns>
        public int SyncState(Action<string, eOutputType> output)
        {
            int res = 0;

            Zy_status_tb zy_status_tb = this.EquDber.Entity<Zy_status_tb>();
            if (zy_status_tb != null)
                commonDAO.SetSignalDataValue(this.MachineCode, eSignalDataName.系统.ToString(), ChangeSystemStatus(zy_status_tb.Samready).ToString());

            foreach (Zy_state_tb entity in this.EquDber.Entities<Zy_state_tb>())
            {
                if (entity.Devicename == GlobalVars.EquHeartbeatName) continue;
        ...
        private eEquInfAutoMakerSystemStatus ChangeSystemStatus(int status)
        {
            eEquInfAutoMakerSystemStatus res;

            Enum.TryParse<eEquInfAutoMakerSystemStatus>(status.ToString(), out res);
