...
namespace CMCS.Common.DAO
{
	...
	public class CommonDAO
	{
		...
		public string GetCMEquipMentNewChildCode(string parentCode)
		{
			string strNewCode = parentCode;

			for (int i = 1; i < 100; i++)
			{
				strNewCode = parentCode + i.ToString().PadLeft(2, '0');
				//判断该编码是否已经存在
				if (!IsExistCMEquipMentCode(strNewCode)) break;
			}
			return strNewCode;
		}
		...
		/// 判断系统编码是否存在
		/// </summary>
		/// <param name="fuelCode">编码</param>
		/// <returns></returns>
		public bool IsExistCMEquipMentCode(string code)
		{
			int entity = Dbers.GetInstance().SelfDber.Count<CmcsCMEquipment>("where NodeCode=:NodeCode", new { NodeCode = code });
			if (entity > 0)
				return true;
			return false;
		}
		...
		/// 判断名称是否存在
		/// </summary>
		/// <param name="fuelName">名称</param>
		/// <returns></returns>
		public bool IsExistCMEquipMentName(string name, string id)
		{
			int entity = Dbers.GetInstance().SelfDber.Count<CmcsCMEquipment>("where EquipmentName=:EquipmentName and Id<>:Id", new { EquipmentName = name, Id = id });
			if (entity > 0)
				return true;
			return false;
		}
		...
		/// <param name="fuelName">名称</param>
		/// <returns></returns>
		public bool IsExistCMEquipMentCode(string code, string id)
		{
			int entity = Dbers.GetInstance().SelfDber.Count<CmcsCMEquipment>("where EquipmentCode=:EquipmentCode and Id<>:Id", new { EquipmentCode = code, Id = id });
			if (entity > 0)
				return true;
			return false;
		}
