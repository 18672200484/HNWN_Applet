...
namespace CMCS.CarTransport.Queue.Frms.Transport.BuyFuelTransport
{
    public partial class FrmBuyFuelTransport_List : MetroAppForm
    {
        ...
        public void BindData()
        {
            ...
            CurrExportData = Dbers.GetInstance().SelfDber.Entities<CmcsBuyFuelTransport>(tempSqlWhere + " order by CreateDate desc", param);
        ...
        private void btnReportExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < CurrExportData.Count; i++)
                {
                    ...
                    Mysheet1(sheetl, i + 1, 3, "华能渭南");
                    Mysheet1(sheetl, i + 1, 4, entity.FuelKindName);
                    Mysheet1(sheetl, i + 1, 5, entity.GrossWeight.ToString());
                    Mysheet1(sheetl, i + 1, 6, entity.TareWeight.ToString());
                    Mysheet1(sheetl, i + 1, 7, entity.DeductWeight.ToString());
                    Mysheet1(sheetl, i + 1, 8, entity.SuttleWeight.ToString());
                    Mysheet1(sheetl, i + 1, 9, entity.TareTime.Year < 2000 ? "" : entity.TareTime.ToString("yyyy-MM-dd HH:mm:ss"));
                }

                #region 合计
                Mysheet1(sheetl, CurrExportData.Count + 1, 0, "合计");
                Mysheet1(sheetl, CurrExportData.Count + 1, 1, CurrExportData.Count + "车");
                Mysheet1(sheetl, CurrExportData.Count + 1, 5, Math.Round(CurrExportData.Sum(a => a.GrossWeight), 2).ToString());
                Mysheet1(sheetl, CurrExportData.Count + 1, 6, Math.Round(CurrExportData.Sum(a => a.TareWeight), 2).ToString());
                Mysheet1(sheetl, CurrExportData.Count + 1, 7, Math.Round(CurrExportData.Sum(a => a.DeductWeight), 2).ToString());
        ...
        {
            if (superGridControl1.PrimaryGrid.GetCell(e.GridCell.GridRow.Index, superGridControl1.PrimaryGrid.Columns["clmId"].ColumnIndex) == null) return;
        ...
        private void superGridControl1_DataBindingComplete(object sender, DevComponents.DotNetBar.SuperGrid.GridDataBindingCompleteEventArgs e)
        {
            ...
            object[] sum = new object[17] { "合计", "", "", "", "", "车数：" + CurrExportData.Count, "", "", "", CurrExportData.Sum(a => a.TicketWeight), "", CurrExportData.Sum(a => a.GrossWeight), CurrExportData.Sum(a => a.TareWeight), CurrExportData.Sum(a => a.DeductWeight), CurrExportData.Sum(a => a.SuttleWeight), "", "" };
        ...
        private void tsmiPrint_Click(object sender, EventArgs e)
        {
            ...
            if (entity != null)
            {
                ...
                ReportPrint.GetInstance().PrintBuyFuelTransport(entity);
            }
        }
    }
}
