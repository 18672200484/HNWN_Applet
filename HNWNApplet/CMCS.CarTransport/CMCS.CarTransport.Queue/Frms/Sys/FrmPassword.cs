...
namespace CMCS.CarTransport.Queue.Frms.Sys
{
    public partial class FrmPassword : DevComponents.DotNetBar.Metro.MetroForm
    {
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            //User user = Dbers.GetInstance().SelfDber.Entities<User>(" where UserAccount=:UserAccount and MDPassword=:MDPassword", new { UserAccount = GlobalVars.LoginUser.UserAccount, MDPassword = MD5Util.Encrypt(txtUserPassword.Text.Trim()) }).FirstOrDefault();
            CmcsUser user = Dbers.GetInstance().SelfDber.Entities<CmcsUser>(" where UserAccount=:UserAccount and UserPassword=:UserPassword", new { UserAccount = GlobalVars.LoginUser.UserAccount, UserPassword = txtUserPassword.Text.Trim() }).FirstOrDefault();
            if (user != null)
            {
                ...
                Dbers.GetInstance().SelfDber.Execute("update " + DapperDber.Util.EntityReflectionUtil.GetTableName<CmcsUser>() + " set UserPassword=:UserPassword where Id=:Id", new { UserPassword = txtUserNewPassword.Text.Trim(), Id = user.Id });
