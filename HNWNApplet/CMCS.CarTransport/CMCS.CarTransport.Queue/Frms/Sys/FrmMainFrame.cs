...
namespace CMCS.CarTransport.Queue.Frms.Sys
{
	public partial class FrmMainFrame : MetroForm
	{
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void OpenCMEquipMent_Click(object sender, EventArgs e)
		{
			string uniqueKey = CMCS.CarTransport.Queue.Frms.BaseInfo.CmequiPment.FrmCmequiPment_List.UniqueKey;

			if (FrmMainFrame.superTabControlManager.GetTab(uniqueKey) == null)
			{
				CMCS.CarTransport.Queue.Frms.BaseInfo.CmequiPment.FrmCmequiPment_List frm = new CMCS.CarTransport.Queue.Frms.BaseInfo.CmequiPment.FrmCmequiPment_List();
				FrmMainFrame.superTabControlManager.CreateTab(frm.Text, uniqueKey, frm, true, true);
			}
			else
				FrmMainFrame.superTabControlManager.ChangeToTab(uniqueKey);
