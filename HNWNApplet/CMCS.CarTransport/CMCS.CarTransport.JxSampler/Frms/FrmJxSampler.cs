	...
	public partial class FrmJxSampler : DevComponents.DotNetBar.Metro.MetroForm
	{
		...
		public CmcsBuyFuelTransport CurrentBuyFuelTransport
		{
			...
			set
			{
				...
				if (value != null)
				{
					commonDAO.SetSignalDataValue(this.SamplerMachineCode, eSignalDataName.当前运输记录Id.ToString(), value.Id);
				...
				{
					commonDAO.SetSignalDataValue(this.SamplerMachineCode, eSignalDataName.当前运输记录Id.ToString(), string.Empty);
		...
		public CmcsAutotruck CurrentAutotruck
		{
			...
			set
			{
				...
				if (value != null)
				{
					commonDAO.SetSignalDataValue(this.SamplerMachineCode, eSignalDataName.当前车Id.ToString(), value.Id);
					commonDAO.SetSignalDataValue(this.SamplerMachineCode, eSignalDataName.当前车号.ToString(), value.CarNumber);
				...
				{
					commonDAO.SetSignalDataValue(this.SamplerMachineCode, eSignalDataName.当前车Id.ToString(), string.Empty);
					commonDAO.SetSignalDataValue(this.SamplerMachineCode, eSignalDataName.当前车号.ToString(), string.Empty);
				...
				switch (this.CurrentFlowFlag)
				{
							...
							if (this.CurrentAutotruck.IsUse == 1)
							{
								if (this.CurrentAutotruck.CarriageLength > 0 && this.CurrentAutotruck.CarriageWidth > 0 && this.CurrentAutotruck.CarriageBottomToFloor > 0)
								{
									...
									if (unFinishTransport != null)
									{
										...
										{
											commonDAO.SetSignalDataValue(SamplerMachineCode, eSignalDataName.矿发量.ToString(), this.CurrentBuyFuelTransport.TicketWeight.ToString());
											// 判断路线设置
											string nextPlace;
					...
					case eFlowFlag.发送计划:
						...
						if (this.SamplerSystemStatus == eEquInfSamplerSystemStatus.就绪待机)
						{
							...
							if (sampling != null)
							{
								...
								commonDAO.SetSignalDataValue(SamplerMachineCode, eSignalDataName.采样编码.ToString(), sampling.SampleCode);
								commonDAO.SetSignalDataValue(SamplerMachineCode, eSignalDataName.采样点数.ToString(), this.CurrentSampleCMD.PointCount.ToString());
					...
					case eFlowFlag.等待采样:
						...
						if (qCJXCYSampleCMD.ResultCode == eEquInfCmdResultCode.成功.ToString())
						{
							...
							{
								commonDAO.SetSignalDataValue(SamplerMachineCode, eSignalDataName.采样时间.ToString(), qCJXCYSampleCMD.StartTime.ToString("HH:mm:ss"));
		...
		void ResetBuyFuel()
		{
			...
			commonDAO.SetSignalDataValue(SamplerMachineCode, eSignalDataName.采样编码.ToString(), "CY2020001002");
			commonDAO.SetSignalDataValue(SamplerMachineCode, eSignalDataName.采样点数.ToString(), "3");
			commonDAO.SetSignalDataValue(SamplerMachineCode, eSignalDataName.采样时间.ToString(), DateTime.Now.ToString("HH:mm:ss"));
			commonDAO.SetSignalDataValue(SamplerMachineCode, eSignalDataName.矿发量.ToString(), "55.36");
		}
