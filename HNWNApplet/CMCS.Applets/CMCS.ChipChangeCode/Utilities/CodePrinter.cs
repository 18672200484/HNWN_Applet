...
{
    Font FontContent = new Font("宋体", 14, FontStyle.Regular, GraphicsUnit.Pixel);
    ...
    void prtdoc_PrintPage(object sender, PrintPageEventArgs e)
    {
        if (_listCode != null)
        {
            if (_listCode.Count > 0)
            {
                ...
                bc.WriteBar(_listCode[PageIndex], 90, 15, 160, 160, g);
                float titleWidth = g.MeasureString(_listCode[PageIndex], FontContent).Width;
                g.DrawString(_listCode[PageIndex], FontContent, Brushes.Black, new PointF((320 - titleWidth) / 2, 10));
